...
// 2. Mis Propios marcadores
  ...
  if (isNaN(Latitud) || isNaN(Longitud)) {
    ...
    return;
      ...
      enviarDatosXML(Nombre,Latitud,Longitud);
...
function enviarDatosXML(Nombre,Latitud,Longitud){

  let datos ="nombre=" + encodeURIComponent(Nombre) +"&latitud="+encodeURIComponent(Latitud)+"&longitud="+encodeURIComponent(Longitud);
  $.ajax(
     
    {
        url:"http://localhost/ejerciciosEntornoCliente/Tarea07/03_JQUERY_XML/lista.php?"+datos,
        type:"GET",
        success: (responseXML)=>{ 
                cargarTabla(responseXML);
              $("#estado").text("Cargada.")},

        error: (error)=>{  console.error("Error en la petición:", error);
            $("#estado").text("Error al cargar los datos.");},
        complete:()=>{  console.log("Petición completada.");console.log("peticion Ajax enviada");},
       async:true,
    }
)
...
function cargarTabla (responseXML) {

if(!responseXML){
  console.warn("Los datos del xml no han sido recibidos");
  const $nodoParrafo = $("p").addClass("no-cargada").
  text("Los datos no han sido Cargados, haz click en cualquier parte del mapa para establecer las coordenadas");
  $("#resultados").empty().append($nodoParrafo);
  return;
}

  //document.getElementById("resultados").innerHTML = peticion.responseText ; // propiedad del xmlhttprequest
  let tabla =""
  let contenido = $(responseXML).find("LOCALIZACION").get();
  tabla += "<table>";
  tabla +="<thead>";
  tabla +="<th>" + "Nombre" + "</th>" + "<th>" +"Latitud"+ "</th>" +  "<th>" + "Longitud" + "</th>";

  for (let i = 0 ; i < contenido.length; i++){
    let localizacion = contenido[i];
    let nombre = $(localizacion).find("NOMBRE").text(); 
    let latitud = $(localizacion).find("LATITUD").text();
    let longitud = $(localizacion).find("LONGITUD").text();
