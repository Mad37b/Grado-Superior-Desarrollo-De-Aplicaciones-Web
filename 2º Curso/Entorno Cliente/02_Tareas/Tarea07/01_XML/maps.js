...
function enviarDatosXML(Nombre,Latitud,Longitud){
  ...
  peticion.open('GET', "http://localhost/ejerciciosEntornoCliente/Tarea07/01_XML/lista.php?"+datos);
  peticion.send();
...
return;
  ...
  let tabla =""
  let contenido = peticion.responseXML.documentElement.getElementsByTagName("LOCALIZACION");
  tabla += "<table>";
  tabla +="<thead>";
  tabla +="<th>" + "Nombre" + "</th>" + "<th>" +"Latitud"+ "</th>" +  "<th>" + "Longitud" + "</th>";

  for (let i = 0 ; i < contenido.length; i++){
    let nombre = contenido[i].getElementsByTagName("NOMBRE")[0].innerHTML; 
    let latitud = contenido[i].getElementsByTagName("LATITUD")[0].innerHTML;
    let longitud = contenido[i].getElementsByTagName("LONGITUD")[0].innerHTML;
    tabla += "<tr>";
    tabla += `<td>${nombre}</td><td>${latitud}</td><td>${longitud}</td>`;
    tabla += "</tr>";
  }
