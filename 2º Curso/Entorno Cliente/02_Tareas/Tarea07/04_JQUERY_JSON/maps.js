/**
 *  Tarea 07 : Ajax y JSON con JQUERY
 * 
...
// 2. Mis Propios marcadores
    ...
    // Variables
  let borrarMarcadores = document.getElementById("borrarMarcadores"); // No lo uso 
  let arrayMarcadores = [] // SI LO USO 
...
function enviarDatosJSON(Nombre,Latitud,Longitud){
  ...
  $.ajax({
        ...
        url:"http://localhost/ejerciciosEntornoCliente/Tarea07/04_JQUERY_JSON/lista.php?"+datos,
        type:"GET",
        dataType: "json",
        success: (responseJSON)=>{ 
                cargarTabla(responseJSON);
              $("#estado").text("Cargada.")},
...
function cargarTabla (responseJSON) {

if(!responseJSON){
  console.warn("Los datos del JSON no han sido recibidos");
  ...
  for (let i = 0 ; i < responseJSON.length; i++){
    let localizacion = responseJSON[i];
    let nombre = localizacion.nombre; 
